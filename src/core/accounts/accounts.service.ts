...
export class AccountsService {
  ...
  async getOrCreate(
    ...
    const phoneNumber = contacts.find(
      (contact) => contact.channel === NotificationChannel.Phone,
    )?.address;
    if (!phoneNumber) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await manager.insert(
        Contact,
        contacts.map((contact) => ({ ...contact, accountId })),
      );
    });
  ...
  async getBy({
    ...
    const item = await this.accounts.findOne({
      where: filter,
      relations: ['contacts'],
  ...
  async update(
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager.update(Account, { id: parseInt(id) }, account);
