...
export class QrsService {
  ...
  async select(accountId?: string | null): Promise<QR[]> {
    const filter: FindOptionsWhere<QR> = {
      accountId: accountId ? parseInt(accountId) : IsNull(),
    };

    return this.qrs.find({
      where: accountId === undefined ? undefined : filter,
    });
  }

  async get(code: string): Promise<QR> {
    const item = await this.qrs.findOne({
      where: { code },
      relations: ['account', 'account.contacts'],
  ...
  async link(code: string, accountId: string, fields: BaseQR): Promise<QR> {
    ...
    const updateResult = await this.qrs.update(
      { code, accountId: IsNull() },
      { ...fields, accountId: parseInt(accountId) },
    );
